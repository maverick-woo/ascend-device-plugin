...
2. Configure `device-share` according to the official guide, and install ascend-docker-runtime with the `ascend` RuntimeClass. Workload images need matching CANN and an executable `/usr/bin/systemd-detect-virt` with its dependencies. The PyTorch example also needs `torch_npu`; the vLLM example needs vLLM-Ascend.
